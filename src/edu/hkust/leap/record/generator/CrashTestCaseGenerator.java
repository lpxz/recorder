package edu.hkust.leap.record.generator;
...
import edu.hkust.leap.record.RecordMain;
...
public class CrashTestCaseGenerator {
	...
	// "import edu.hkust.leap.replay.tracer.*;",
	// "import edu.hkust.leap.replay.control.*;"
		"import java.lang.reflect.Method;"	,
	"import edu.hkust.leap.replayer.Scheduler;",
	...
	public GeneratedCode codeGeneration(String packageName, String traceFile_)
		...
		String traceFileName_accessVector = traceFile_
				+ "_accessVector.trace.gz";
		String traceFileName_threadNameToIdMap = traceFile_
		...
		String quotedTraceFileName_accessVector = traceFileName_accessVector
				.replace("\\", "\\\\");
		String quotedTraceFileName_threadNameToIdMap = traceFileName_threadNameToIdMap
				.replace("\\", "\\\\");

		generatedCodeString += "\n\tpublic static void setUp() throws Exception {\n";

		generatedCodeString += "\t\tTraceReader.readTrace(1,\""
				+ quotedTraceFileName_accessVector + "\");\n";
		generatedCodeString += "\t\tTraceReader.readTrace(2,\""
				+ quotedTraceFileName_threadNameToIdMap + "\");\n";

		generatedCodeString += "\t}\n";

		generatedCodeString += "\n\tpublic static void main(String[] args) throws Throwable {\n";
		generatedCodeString += "\n\t\tsetUp();\n";
		generatedCodeString += "\n\t\tThread.currentThread().setName(\"leap-main\");\n";
		generatedCodeString += "\n\t\t(new Scheduler()).start();\n";
		//Thread.currentThread().setName("leap-main");
...
//				+ Util.MAIN_THREAD_NAME + "\"){\n" + "\t\t\t"
//				+ "public void run(){\n";
//		generatedCodeString += createNewThreadString;

		String arg = "{";
		for (int i = 0; i < RecordMonitor.mainargs.length; i++) {
			if (i < RecordMonitor.mainargs.length - 1)
				arg += "\"" + RecordMonitor.mainargs[i] + "\"" + ",";
			else
				arg += "\"" + RecordMonitor.mainargs[i] + "\"";
		...
		callMethodString +=	"\t\t\t\tClass<?> c = Class.forName("+ 
		"\"" +RecordMonitor.mainClass+ "\"" + ");" +
		...
		generatedCodeString += callMethodString;
		generatedCodeString += "\n\t\t\t\t} catch (Throwable e) {";
		generatedCodeString += "\n\t\t\t\t\te.printStackTrace();";
		generatedCodeString += "\n\t\t\t\t}\n";
	...
	private String generateStackTrace(GeneratedCode generatedCode) {
		...
		Throwable exception = RecordMonitor.crashedException;
	...
	static public void main(String args[]) throws IOException,
		...
		CrashTestCaseGenerator ctcg = new CrashTestCaseGenerator();
		GeneratedCode generetedCode = null;
